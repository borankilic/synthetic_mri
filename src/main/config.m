function config = config()
    ...
    config.paths.mapvbvd_path = '../../scripts/mapVBVD-main/';    %DO NOT MODIFY unless you know what you are doing
    config.paths.romeo_script_path = '../../scripts/romeo.jl';   

    % --- Sequence Parameters ---
    config.params.sequence = 'GRE';
    ...
    config.params.TI_values = [ {10; 20}, {15; 20}, {15; 30}];  % For MP2RAGE each combination should be a cell representing first and second inversion recovery times. 
   ...
   config.params.smap_params = struct('ncalib', 40,...                  % Size of the Auto Calibration Region (ACS) is taken as ncalib x calib (or if [nx, ny]
    ...
    config.options.threshold_tmap = 0.95;    % Threshold for brain extraction using tissue maps. 
    config.options.threshold_tmap = 0.5;     % Threshold used for BET. Fractional intensity threshold (0->1). Smaller values give larger brain outline estimates 
    ...
    config.options.SNR_dB = 20;    % Add noise based on SNR of resulting image. Set to 1 for no noise
    config.options.num_images = 1; % Change for generating multiple weighted images (with random noise distributions) 
